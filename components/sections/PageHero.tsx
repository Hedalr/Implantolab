...
export function PageHero({ eyebrow, title, intro }: PageHeroProps) {
  return (
    <section className="bg-[var(--bg)] border-b border-[var(--line)]">
      <Container size="wide" className="pt-10 sm:pt-16 md:pt-24 pb-12 sm:pb-16 md:pb-20">
        <div className="grid gap-6 sm:gap-10 lg:grid-cols-12">
          <Reveal className="lg:col-span-7">
            ...
            <h1 className="text-display text-display-page mt-4 sm:mt-6 text-balance">
              {title}
            </h1>
          </Reveal>
          <Reveal delay={120} className="lg:col-span-5 lg:pt-2">
            <p className="text-base sm:text-lg text-[var(--ink-muted)] leading-relaxed text-pretty">
              {intro}
            </p>
          </Reveal>
        </div>
